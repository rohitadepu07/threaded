  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetTrigger asChild>
        <Button variant="outline" size="icon" className="relative border-antique-gold/40 bg-background/80 hover:bg-accent/10">
            ...
            <>
              <div className="flex-1 overflow-y-auto pr-2 min-h-0">
                <div className="space-y-4">
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-3 rounded-md border border-border bg-card">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-display font-medium truncate text-foreground">{item.product.node.title}</h4>
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t border-border bg-background">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-display font-bold text-foreground">{items[0]?.price.currencyCode || '$'} {totalPrice.toFixed(2)}</span>
                </div>
